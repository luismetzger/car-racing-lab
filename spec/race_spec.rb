require 'spec_helper'

RSpec.describe Race do
  subject(:race) { Race.new }

  describe '::new' do
    it 'instantiates two cars' do
      expect(race.cars[0].class).to eq Car
      expect(race.cars[1].class).to eq Car
    end
    it 'accelerates the cars to random speeds' do
      expect(race.cars[0].speed).to_not eq race.cars[1].speed
      expect(race.cars[0].speed).to_not be 0
      expect(race.cars[1].speed).to_not be 0
    end
  end
  describe '#winner' do
    it 'returns the winner' do
      expect(race.winner.speed).to_not eq(race.loser)
    end
    it 'is not the loser' do
      expect(race.winner.speed).to_not eq(race.loser.speed)
    end
  end

  describe '#loser' do
    it 'returns the loser' do
      expect(race.cars[0]).to_not eq(race.cars[1])
    end
    it 'is not the winner' do
      expect(race.loser.speed).to_not eq(race.winner.speed)
    end
  end
end
